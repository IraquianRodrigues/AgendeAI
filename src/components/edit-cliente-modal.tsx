...
export function EditClienteModal({ isOpen, onClose, cliente }: EditClienteModalProps) {
  ...
  // Load cliente data when modal opens
  useEffect(() => {
    if (cliente && isOpen) {
      ...
      if (cliente.data_nascimento) {
        setDataNascimento(formatBirthDate(cliente.data_nascimento));
      } else {
        setDataNascimento("");
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["cliente", cliente.telefone] });
